...
public interface IPrimaryFrameController {
	 ...
	 * @param path the path that leads to the binary file from which the data will be loaded.
	 * If null, the data will be loaded from the home directory (if a file .gym is present).
	 */
	void cmdLoad(final String path);
	
	/**
	 * Saves the model's data into a .gym file (binary file).
	 * @param path the path that leads to the binary file in which the data will be stored.
	 * If null, the data will be stored in a .gym file in the home directory (if such file is not present in the home directory, it will be generated automatically).
	 */
