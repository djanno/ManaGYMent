...
public class SendEmailPanelController implements ISendEmailPanelController{
	...
	public void cmdSend(final String subject, final String body, final boolean employee, final boolean subscriber, final boolean exSubscriber, final char[] password) {
		try {
			new SenderEmail(subject, body, getListRecipients(employee, subscriber, exSubscriber), this.model.getUser(this.getLoggedUser()).getEmail(), password).sendEmail();
    ...
    /**
     * Returns the username of the user that is currently logged in. The purpose of this method is to avoid code duplication.
