...
public class SubscriberAddController extends MemberAddController implements ISubscriberAddController {
     ...
     */
    public SubscriberAddController(final PrimaryFrame frame, final ISubscriberPanel subscriberView, final IGym gym,
            final TableSubscribersController tableSubscribersController) {
        super();
        this.frame = frame;
    ...
    public void cmdSave(final Map<IFormField, String> mapToPass, final Date subscriptionDate, final Date expirationDate,
            final DefaultListModel<String> list) throws IllegalArgumentException {
     ...
     * @throws Exception
     */
    private Subscriber createSubscriber(final Map<IFormField, String> mapToPass, final Calendar subscriptionDate, final Calendar expirationDate,
            final DefaultListModel<String> list) throws Exception {
    ...
    private void checkForDateExceptions(final Calendar date, final Calendar lowerLimit) {
        ...
        if (date.get(Calendar.YEAR) < lowerLimit.get(Calendar.YEAR)) {
            ...
            else if (date.get(Calendar.MONTH) == lowerLimit.get(Calendar.MONTH)
                    && date.get(Calendar.DAY_OF_MONTH) < lowerLimit.get(Calendar.DAY_OF_MONTH)) {
                throw new IllegalArgumentException(INVALID_DATES);
            }
        }
    }
