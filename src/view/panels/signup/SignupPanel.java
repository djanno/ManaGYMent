...
public class SignupPanel extends Background implements ISignupPanel, ActionListener, KeyListener {
	...
	public SignupPanel(final String path) {
		...
		final GridBagConstraints cnst = new GridBagConstraints(0, 1, 1, 1, 0.5, 0.028, GridBagConstraints.CENTER, GridBagConstraints.NONE, emptyInsets, 0, 0);
		...
		cnst.gridy++;
		this.add(this.signupButton, new GridBagConstraints(0, cnst.gridy, 1, 1, 0.5, 0.3, GridBagConstraints.CENTER, GridBagConstraints.NONE, emptyInsets, 0, 10));
		this.add(this.cancelButton, new GridBagConstraints(1, cnst.gridy, 1, 1, 0.5, 0.3, GridBagConstraints.CENTER, GridBagConstraints.NONE, emptyInsets, 0, 10));
	}
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if(source == this.signupButton) {
			final Map<ISignupField, String> mapToPass = this.strategy.fields().stream().collect(Collectors.toMap(field -> field, field -> this.map.get(field).getText()));
