...
public class SignupStrategy implements ISignupStrategy {
	...
	public enum SignupField implements ISignupField {
		...
		NAME ("nome: ", 20, false, x->(x.length() > 1 && x.length() < 20), "max 20 caratteri."),
		SURNAME ("cognome: ", 20, false, x->(x.length() > 1 && x.length() < 20), "max 20 caratteri."),
		GYM_NAME ("nome palestra: ", 20, false,  x->(x.length() > 1 && x.length() < 20), "max 20 caratteri."),
		USERNAME ("username: ", 20, false, x->(x.length() > 4 && x.length() < 20), "max 20 caratteri."),
		PASSWORD ("password: ", 20, true, x->(x.length() > 7), "almeno 8 caratteri."),
		CONFIRM_PASSWORD ("conferma password: ", 20, true, x->(x.length() > 7), "reinserire la propria password."),
		EMAIL ("email: ", 20, false, x->(Pattern.compile(SignupField.EMAIL_PATTERN).matcher(x).matches() && (x.contains("@gmail.com") || x.contains("@yahoo.com") || x.contains("@yahoo.it"))), "solo gmail.com, yahoo.com o yahoo.it."),
