...
public class FieldsCommon implements IFormStrategy {
    ...
    public enum EnumFieldsCommon implements IFormField {
        ...
        NOME("Nome", x -> x.length() > 1), COGNOME("Cognome", x -> x.length() > 1), CODICE_FISCALE("Codice fiscale", x -> x.length() == 15), INDIRIZZO(
                "Indirizzo", x -> x.length() > 7), TELEFONO("Telefono", x -> isNumber(x)), EMAIL("E-Mail", x -> Pattern
                .compile(EnumFieldsCommon.EMAIL_PATTERN).matcher(x).matches());
